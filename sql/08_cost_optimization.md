...
> この章で実行するファイル: `sql/08_cost_optimization.sql`
...
### Snowflake の 2 種類のキャッシュとコストの関係

| キャッシュ | 場所 | Warehouse 必要？ | 有効期間 | ヒット条件 |
|---|---|---|---|---|
| **Result Cache（結果キャッシュ）** | Cloud Services 層 | 不要（0 クレジット） | 24 時間 | 完全に同じ SQL・同じデータ |
| **Local Disk Cache（仮想 WH キャッシュ）** | Warehouse の SSD | 必要 | Warehouse 起動中 | 同じテーブルデータの再スキャン |

**コスト最適化との関係**:
- Result Cache ヒット時は Warehouse が起動不要 → クレジット消費ゼロ
- ダッシュボードで同じ集計クエリを何度も叩く場合、Result Cache が効けば自動でコスト削減できる
- Warehouse を再起動すると Local Disk Cache がクリアされるので、頻繁な再起動はパフォーマンス低下につながる

> 詳細・ハンズオン（Result Cache を無効化して比較）は **付録 A5** を参照してください。

---

### クラスタリングキーとコスト削減

Clustering Key を設定すると、Snowflake がデータを物理的に再整列し、
WHERE フィルタで読み飛ばせるパーティションが増える（＝スキャン量が減る＝コスト削減）。

**有効な場面**:
- テーブルが数億行以上の大規模データ
- 特定の列（日付・カテゴリなど）で絞り込むクエリが多い

**有効でない場面（設定するとかえってコスト増）**:
- 小〜中規模テーブル（学習環境の 200 件など）
- ほぼ全件スキャンするクエリが中心

> 詳細・設定 SQL・コスト確認クエリは **付録 A5** を参照してください。
...
```sql
-- auto_suspend の設定を確認（SHOW WAREHOUSES の auto_suspend 列を見る）
show warehouses like 'LEARN_WH';
```
