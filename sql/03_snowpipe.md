...
### Step 2-b: SnowCLI でアップロードする

ブラウザを使わず CLI で進める場合は、同じ stage に `PUT` できます。
...
`absolute/path` は自分のローカルパスに置き換えてください。`list @RAW.EVENT_STAGE;` でファイルが見えれば成功です。
...
SELECT
  PARSE_JSON(
    ...
    || '"items":[{"sku":"...", "product_name":"SKUごとに固定名", "category":"SKUごとに固定カテゴリ",
...
> また、後続の `dbt` 章で `dim_products` を安定して作れるよう、同じ `sku` には同じ `product_name` / `category` が入る前提で生成します。
