...
> この章で実行するファイル: `sql/10_end_to_end_pipeline.sql`

## この章で学ぶこと

- 01〜09章で構築したパイプライン全体を俯瞰する
...
| 第8章 | `08_ai_sql.sql` | STAGING.REVIEWS |
...
datasets/events_sample.json
  ...
  MART.FACT_PURCHASE_EVENTS
        ...
        │
        └──→ STAGING.REVIEWS → AI_COMPLETE / AI_CLASSIFY（08章）
```
...
### Check 4: AI でレビューを要約（← 08章 STAGING.REVIEWS のデータ）
...
| STAGING | `REVIEWS` | AI 関数で処理するテキストデータ | 08章 |
...
- **dbt を使う**: SQL テンプレート・テスト・ドキュメント化を自動化（→ 11章）
- **Airflow を使う**: 外部システムとの連携・より複雑なワークフロー管理（→ 12章）
...
7. **コスト最適化**（07章）: Warehouse 設定・STAGING テーブルの活用
8. **AI 関数**（08章）: Cortex の AI_COMPLETE・AI_CLASSIFY・AI_EXTRACT
