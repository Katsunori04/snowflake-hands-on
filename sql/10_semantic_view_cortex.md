...
**呼び出し方法は環境によって異なります**（SQL 関数では呼び出せない点に注意）:

| 環境 | 方法 | 可否 |
|---|---|---|
| Snowpark (Python) | SQL 関数 / Snowpark API | ❌ 非対応 |
| Streamlit in Snowflake | `_snowflake.send_snow_api_request()` | ✅ 対応 |
| 外部アプリ（ローカル PC など） | REST API 直接呼び出し | ✅ 対応 |
| Snowsight | GUI（AI & ML → Cortex Analyst） | ✅ 対応 |

REST API エンドポイント: `POST /api/v2/cortex/analyst/message`
...
Cortex Analyst は **REST API** で呼び出します（Snowpark からは直接呼び出せません）。
以下の Python スクリプトは**ローカル PC などの外部環境**で実行してください。
Streamlit in Snowflake から呼び出す場合は `_snowflake.send_snow_api_request()` を使います（後述）。
...
## 付録: Streamlit in Snowflake から Cortex Analyst を呼び出す

Snowsight の **Streamlit** タブでアプリを作成する場合、`_snowflake.send_snow_api_request()` を使って
Cortex Analyst を呼び出せます（外部への `requests` 呼び出しは不要）。
...
st.title("Cortex Analyst チャット")

question = st.text_input("質問を入力してください", "都道府県別の売上合計を教えてください")
...
if st.button("送信"):
    payload = {
        ...
        "semantic_view": "HANDS_ON_DB.MART.SEM_PURCHASE_EVENTS"
    }
    ...
    for block in result["message"]["content"]:
        if block["type"] == "text":
            st.write("**AI の解釈:**", block["text"])
        elif block["type"] == "sql":
            st.code(block["statement"], language="sql")
```

> **ポイント**: `_snowflake` モジュールは Streamlit in Snowflake 専用。ローカル環境では動作しない。
> **参考**: [Streamlit in Snowflake](https://docs.snowflake.com/ja/developer-guide/streamlit/about-streamlit) / [send_snow_api_request](https://docs.snowflake.com/ja/developer-guide/streamlit/calling-snowflake-apis)
